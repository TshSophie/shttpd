...
// 初始化socket和epoll
void initsocket(int epfd,int port,int backlog);
...
void show_help(){
  ...
  char *str = "Usage: shttpd [options] [-- <shttpdapp> [shttpd app arguments]]\n" \
    ...
    " -?,-h     this help\n" \
    " -v             show version\n" \
    " -d <directory> chdir to directory before using shttpd\n" \
...
{ 
  ...
  char *endptr = NULL;
  // char *dir = NULL;
  char *rootdir = "/home/sophie/Linux_C2/project/webserver/html";
  
  port = SERV_PORT;

  // dir = (char *)get_current_dir_name();  
  // sprintf(rootdir,"%s/html/",dir);

  while (-1 != (o = getopt(argc, argv, "p:?hd:v"))) {
    switch(o) {   
    case 'd': strcpy(rootdir,optarg); break;    
    case 'p': port = strtol(optarg, &endptr, 10);/* port */
      if (*endptr) {
        fprintf(stderr, "shttpd: invalid port: %u\n", (unsigned int) port);
        return -1;
      }
      break;    
